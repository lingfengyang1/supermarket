  ...
  <div class="shop-info">
    ...
    <div class="shop-middle">
      ...
      <div class="sale-count">
        <!-- 以下值需要换算 因此的话 通过过滤器进行换算足矣 -->
        <p class="count">{{shop.cSells | bigNumberTransform}}</p>
        <p class="text">总销量</p>
      </div>
      <div class="all-goods">
      ...
      <div class="shop-score">
        <table>
          <!-- key取值为index而非item的原因在于item作为对象最终会被vue解析为对象类型 从而导致key重复 无法标识遍历的每一个元素的唯一性 而index则可以 -->
          <tr v-for="(item, index) in shop.score" :key="index">
            <td class="name">{{item.name}}</td>
            <td class="score" :style="item.isBetter&&{color: 'red'}">{{item.score}}</td>
            <td class="cost-per" :style="item.isBetter&&{backgroundColor: 'red'}">{{item.isBetter?"高":"低"}}</td>
          </tr>
        </table>
      </div>
    </div>
